...
import { Trophy, Medal, Star, Crown, Shield, ArrowRight } from "lucide-react";
import { useLeaderboard } from "@/hooks/api/useGamification";
import { TimeFrame } from "@/types/gamification";
...
const LeaderboardContent = ({ timeRange }: LeaderboardContentProps) => {
  const [limit, setLimit] = useState(10);
  const { leaderboard, isLoading, isError } = useLeaderboard(timeRange, limit);
  const [animatePodium, setAnimatePodium] = useState(false);

  useEffect(() => {
    setAnimatePodium(false);
    const timer = setTimeout(() => setAnimatePodium(true), 50);
  ...
  const otherUsers =
    leaderboard?.entries
      .filter((u) => u.rank > 3)
      .sort((a, b) => a.rank - b.rank) || [];
  const userRankDetails = leaderboard?.user_rank_details;
                ...
                <div className="relative z-10">
                  ...
                  <div className="flex items-center gap-2">
                    <p className="text-xs text-secondary-foreground whitespace-nowrap">
                      {userRankDetails.points_to_next_rank !== null
                        ? `${userRankDetails.points_to_next_rank} pts to rank ${
            ...
            {topUsers.map((user) => {
              ...
              const delay =
                user.rank === 1
                  ...
                  ? "delay-100"
                  : "delay-500";
              const translateY = animatePodium
                ? "translate-y-0"
                : "translate-y-20";
              const opacity = animatePodium ? "opacity-100" : "opacity-0";
      ...
      >
        ...
        {otherUsers.map((user) => (
          <div key={user.user_id}>
            <Card className="px-6 py-4 flex items-center gap-4 border-divider bg-card hover:bg-accent/30 transition-all duration-300 relative overflow-hidden group cursor-pointer">
              <div
                className="absolute inset-y-0 left-0 w-1"
                style={{
                  backgroundColor: getMedalColor(user.rank > 3 ? 4 : user.rank),
                }}
              ></div>
        ...
        {leaderboard && leaderboard.entries.length === limit && (
          <Button
            variant="outline"
...
export default function LeaderboardPage() {
  const [activeTab, setActiveTab] = useState<TimeFrame>(TimeFrame.WEEKLY);
  ...
  return (
    <main className="flex flex-col bg-background items-center min-h-screen pb-16 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl pt-14 px-4 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex border border-border rounded-md divide-x divide-border">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setActiveTab(TimeFrame.WEEKLY)}
              className={`min-w-[80px] ${
                activeTab === TimeFrame.WEEKLY
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setActiveTab(TimeFrame.MONTHLY)}
              className={`min-w-[80px] ${
                activeTab === TimeFrame.MONTHLY
                  ? "bg-neutral-700 rounded-md"
                  : "rounded-md hover:bg-neutral-700 hover:text-foreground"
              }`}
            >
            ...
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setActiveTab(TimeFrame.ALLTIME)}
              className={`min-w-[80px] ${
                activeTab === TimeFrame.ALLTIME
                  ? "bg-neutral-700 rounded-md"
                  : "rounded-md hover:bg-neutral-700 hover:text-foreground"
              }`}
            >
        ...
        <div className="mt-6">
          <AnimatePresence mode="wait">
            ...
            >
              <LeaderboardContent timeRange={activeTab} />
            </motion.div>
          </AnimatePresence>
        </div>
      </div>
