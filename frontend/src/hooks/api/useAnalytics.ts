...
function getMockAnalyticsData(timeRange: TimeRange): AnalyticsData {
  ...
  const multiplier =
    timeRange === TimeRange.TODAY ? 0.5 : timeRange === TimeRange.WEEK ? 0.8 : timeRange === TimeRange.MONTH ? 1.2 : 1.5

  return {
    learningEffectivenessScore: Math.round(78 * multiplier > 100 ? 100 : 78 * multiplier),
    currentStreak: 12,
    longestStreak: 21,
...
function generateQuizVsStudyData(timeRange: TimeRange): QuizVsStudyItem[] {
  ...
  for (let i = 0; i < intervals; i++) {
    ...
    const studyAccuracy = Math.min(100, quizAccuracy + 5 + Math.round(Math.random() * 15))
